...
object ReleaseSettings {
  ...
  def rootProjectSettings = 
    ...
    Seq(
      ...
      release.releaseProcess := 
        ...
        createReadmeTask := {
          streams.value.log.info("Creating README.rst")
          val base = baseDirectory.value
          val staticFiles = 
            (base * "*.rst").get.filterNot(_.name == "README.rst").sorted
          val documentationFiles = 
            (base / "documentation" * "*.rst").get.sorted
  ...
  lazy val readme = file("README.rst")

  lazy val updateVersionInReadme = updateVersionInFiles(Seq(readme, file("extra/documentation/README.rst")))
